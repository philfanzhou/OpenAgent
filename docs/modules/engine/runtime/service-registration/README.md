...
|-----------|-------------|
| 自动注册 | 应用启动后写入 Engine 实例信息并维护发现索引 |
...
## Current Status
**已实现** — 注册、心跳、索引自修复与注销均有单元测试覆盖；Router 通过索引批量读取注册项。

## Limits
- 心跳单线程顺序执行，`IsRegistered` 非 volatile
- 注销失败依赖 TTL 自然过期
- 进程异常退出会暂时留下索引成员；Router 发现注册值已过期后会清理该成员
...
- Tests: `Backend/tests/OpenAgent.Engine.Tests/Registry/RedisRegistryTests.cs`
