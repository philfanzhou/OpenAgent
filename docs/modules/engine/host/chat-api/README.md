...
- **MCP 跨系统传输 / 用户分享链接**：大模型调用 `create_file_transfer_url` 返回平台分享链接 `/api/v1/share/{token}`（可选 `mode`：temporary/singleUse/longTerm，`expiresInSeconds` 自定义失效日期）——既可传给需要文件 URL 的第三方 MCP，也可作为下载链接交给用户（须告知 `expiresAt` 有效期与下载限制）；链接不暴露 S3 地址、`objectKey` 或对象存储凭据。
