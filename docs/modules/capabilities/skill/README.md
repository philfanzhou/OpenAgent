...
| 脚本清单 | 上传时扫描包内 `.py` / `.js` / `.mjs` 文件，把相对路径记录到 `SkillInstanceConfig.ScriptNames`（`ScriptCount` 为派生值）；供管理端展示与开启前审阅 |
...
上传的包被视为不可信代码：上传接口（`POST /api/v1/admin/skills/packages` 与 `POST /api/v1/admin/skills/{agentId}/packages`）接受可选 multipart 字段 `scriptExecutionEnabled`，缺省为 false；即使显式传 true，包内没有可执行脚本（`.py` / `.js` / `.mjs`）也会被 400 拒绝。上传时扫描出的脚本相对路径记入 `ScriptNames` 随实例保存。重新上传（覆盖同名 Skill）会把开关重置为 false——新代码需要重新审阅开启。
...
开启后脚本也绝不在 Engine 进程内执行：`SkillScriptRunner` 将脚本与其同目录文件挂载为沙箱 `/input` 输入，经生成的 wrapper（`.py` 生成 `main.py` 以 `runpy` 启动；`.js` / `.mjs` 生成 `main.mjs` 以动态 `import` 启动）在 Bubblewrap 沙箱内运行（无网络、非 root、固定 venv / Node）。约束与 `execute_code` 完全一致：

- 仅 `.py`、`.js`、`.mjs` 脚本（与 Runner 的 Python / JavaScript 入口一致）；shell 与其他解释器不披露、不执行；
- `ExecutionLimits` 输入文件上限（8 个、单文件 10 MiB、总 20 MiB、`main.py` / `main.mjs` 保留名、安全文件名）；
...
- 参数以 JSON 传入 wrapper：Python 映射为 `sys.argv`、JavaScript 映射为 `process.argv`（字符串直传，其余 JSON 编码）。`.js` 以 CommonJS 语义加载，ESM 请使用 `.mjs`。
