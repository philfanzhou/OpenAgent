...
| 资源读取 | 读取文本和 Blob 资源 |
| 传输选择 | 默认 Streamable HTTP，显式 `SSE` 兼容 legacy 服务，受策略约束的 stdio |
...
## Limits
- `Stdio` 生产默认关闭；开启后仅允许策略白名单中的命令、环境变量和工作目录，并视为可信宿主进程
...
- Security: [MCP 与 Skill 执行安全调研](../mcp-skill-execution/RESEARCH.md)
