    ...
    <div v-if="config" class="agent-editor">
      ...
      <section class="agent-editor-section">
        ...
        <el-form label-position="top" class="agent-form-grid">
          ...
          <el-form-item label="API Key Secret 引用" class="span-two"><el-input v-model="config.config.llm.apiKeySecretRef" placeholder="例如 llm:openai-prod" :disabled="Boolean(config.config.llm.provider)" /><small class="form-help">实际密钥由执行端按当前租户从 Secrets 配置或外部 Secret Manager 获取。</small></el-form-item>
        </el-form>
      </section>
