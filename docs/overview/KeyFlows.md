...
- 需要 `create_file_transfer_url` 的两个场景：第三方 MCP 要文件链接时把生成的平台分享链接传给 MCP；用户要直接下载链接时把该链接给出并告知有效期与下载限制（`mode` 支持 temporary/singleUse/longTerm，`expiresInSeconds` 自定义失效日期）。其他文件流程不生成分享链接；链接由 `/api/v1/share/{token}` 匿名核销，不暴露 S3 地址。
