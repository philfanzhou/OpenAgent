...
- **分享链接管理**：`GET /api/v1/agent/files/shares` 查询当前用户的全部分享（含 `isActive` 状态），`DELETE /api/v1/agent/files/shares/{shareId}` 撤销指定分享，撤销后令牌立即失效；模型侧亦有 `list_share_links` / `revoke_share_link` 工具，可由大模型代用户查询与撤销。
