...
public class AuthenticationEndpointTests
{
    ...
    public async Task ProductionJwtEndpoints_DisableCompatibilityPasswordLogin()
    {
        ...
        Assert.True(config.RootElement.GetProperty("domainLogin").GetProperty("enabled").GetBoolean());
        Assert.True(config.RootElement.GetProperty("tenant").GetProperty("enabled").GetBoolean());
