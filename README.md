...
执行聊天前，请先在登录页使用仓库内置的 Development 账号建立身份（见
[`docs/modules/security/auth/README.md`](docs/modules/security/auth/README.md)），再在工作台设置中创建 LLM
Provider 并绑定 Agent。该兼容登录仅适合本地联调，不应直接暴露到公网。
...
Basic 兼容登录严格限制在 Development；它只校验仓库内置账号，不查询真实用户目录。
