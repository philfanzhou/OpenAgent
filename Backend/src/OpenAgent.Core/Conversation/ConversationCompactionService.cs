    ...
    ILoggerFactory loggerFactory) : IConversationCompactionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        AgentRuntimeProfile profile = await runtime.ResolveAsync(
            ...
            user,
            cancellationToken).ConfigureAwait(false);
        IChatClient chatClient = chatClients.Create(profile.Model);
        CompactionStrategy strategy = histories.CreateStrategy(
            profile.Config.ContextPolicy,
            chatClient,
            force: true,
            out CompactionTrigger trigger)
            ?? throw new InvalidOperationException("Conversation compaction strategy is unavailable.");
        ...
        var audited = new AuditedCompactionStrategy(
            strategy,
            trigger,
            ConversationHistoryFactory.ResolveStrategyName(profile.Config.ContextPolicy),
            "Manual",
            context,
            store,
            loggerFactory.CreateLogger<AuditedCompactionStrategy>(),
            recordUnchanged: true);
        List<ChatMessage> messages = record.Messages
