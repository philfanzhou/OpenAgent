...
- **依赖方向、HTTP/流式协议与认证：**
- **租户、Agent ACL、能力权限与 Redis：**
- **PostgreSQL、MinIO、Compose 与部署：**
- **前端与中文使用者文档：**
...
- [ ] 后端单元/架构/集成测试通过
- [ ] 前端类型检查、测试和生产构建通过
- [ ] 涉及 Compose、数据库、Redis、MinIO 或 Keycloak 时，相关 E2E/smoke test 通过
- [ ] 行为、协议或用法变化时，中文 README/docs 已同步
- [ ] 不包含密钥、连接字符串、凭据、用户数据或私有配置
