...
对象存储只需要一个服务地址 `OPENAGENT_S3_SERVICE_URL`（引擎内部访问 S3/MinIO 使用）。
对外下载与分享统一走平台自身的端点（`/api/v1/share/{token}`、认证下载），不再生成或返回
S3 预签名 URL，因此 S3 无需公网域名，也不存在"签名地址与消费地址一致"的配置要求。
默认保持 TLS 校验，不通过关闭校验修复地址/证书问题。
