...
<template>
  <div class="composer" :class="{ 'drag-over': dragDepth > 0 }" @paste="handlePaste" @dragenter="handleDragEnter" @dragover="handleDragOver" @dragleave="handleDragLeave" @drop="handleDrop">
    ...
    <div class="composer-box">
      ...
      <el-input :model-value="props.modelValue" type="textarea" :disabled="props.readOnly" :autosize="{ minRows: 2, maxRows: 10 }" resize="none" :placeholder="props.readOnly ? '会话日志为只读重放' : '向 Agent 发送消息（Shift+Enter 换行）'" @update:model-value="emit('update:modelValue', $event)" @keydown="handleKeydown" />
      ...
      <div class="composer-footer">
        <div class="composer-hints"><el-button text class="file-button" :disabled="props.readOnly" aria-label="添加附件" @click="openFilePicker"><svg viewBox="0 0 20 20" fill="none"><path d="M7 10.8 11.8 6a2.1 2.1 0 1 1 3 3l-6.2 6.2a3.5 3.5 0 0 1-5-5L10 3.8" /></svg><span>添加附件</span></el-button><span>最多 5 个 · 25 MB</span></div>
        <div class="composer-actions"><span class="connection-caption">{{ props.endpointLabel }} · {{ props.endpointUrl || '未配置' }}</span><span class="keyboard-hint">{{ props.readOnly ? '只读重放' : props.loading ? '再次点击停止' : 'Enter 发送' }}</span><el-button type="primary" circle :aria-label="props.loading ? '停止生成' : '发送'" :disabled="props.readOnly || (props.loading ? false : !props.selectedAgentId || !props.selectedLlmProfileId || (!props.modelValue.trim() && !props.pendingFiles.length) || props.pendingFiles.some(item => item.state !== 'ready'))" @click="props.loading ? emit('stop') : emit('send')"><svg v-if="!props.loading" viewBox="0 0 20 20" fill="none"><path d="M10 15V5m0 0L6 9m4-4 4 4" /></svg><span v-else class="stop-icon" aria-hidden="true"></span></el-button></div>
      </div>
    </div>
  </div>
</template>
